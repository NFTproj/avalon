  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            'Campos obrigatórios ausentes (cardId, tokenAddress, stablecoinAddress, tokenPrice, network)',
        },
        { status: 400 },
      )
    }
    ...
    const url = `${apiUrl}/intermediary/deploy`
    ...
    console.error('Erro ao fazer deploy do contrato intermediário:', error)
    return NextResponse.json(
      {
        error:
          error?.message || 'Erro ao fazer deploy do contrato intermediário',
      },
      { status: 500 },
    )
  }
