...
// ---------- Helpers de status (mesma “família” do PixPaymentStatement) ----------
...
function normalizeStatus(raw?: string | number | null): CanonicalStatus {
  ...
  // numérico (ex: 100/200/300/400/500)
  const n = Number(raw)
  if (!Number.isNaN(n)) {
    if (n === 100) return 'WAITING_PAYMENT'
    if (n === 200) return 'PROCESSING'
    if (n === 300) return 'COMPLETED'
    if (n === 400) return 'FAILED'
    if (n === 500) return 'CANCELLED'
  }
  // string
  const s = String(raw).trim().toUpperCase()
  if (['WAITING_PAYMENT', 'WAITING', 'PENDING', 'AWAITING_PAYMENT'].includes(s)) return 'WAITING_PAYMENT'
  if (['PROCESSING', 'IN_PROGRESS'].includes(s)) return 'PROCESSING'
  if (['COMPLETED', 'PAID', 'CONFIRMED', 'SUCCESS'].includes(s)) return 'COMPLETED'
  if (['FAILED', 'ERROR', 'DENIED'].includes(s)) return 'FAILED'
...
export default function OrdersPage() {
  ...
  // novo: filtro de status (default: Concluídas)
  ...
  // normaliza status e aplica filtro client-side (rápido)
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: active ? `${accent}1A` : 'white', // 10% alpha
                  }}
                >
                  {f.label}
                </button>
